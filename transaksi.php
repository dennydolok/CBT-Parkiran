...
$query = mysqli_query($con, "SELECT * FROM tb_daftar_parkir WHERE kode = '$kode_keluar' AND status = '1'");
$data = mysqli_fetch_array($query);

$masuk = $data['hitung_jam_masuk'];
$keluar = date('H');
...
if (isset($_POST['btn_hitung'])) {
  ...
  if ($bayar >= $hasil) {
    $kembali = $bayar - $hasil;
    if ($kembali == 0) {
      $query = mysqli_query($con, "UPDATE tb_daftar_parkir SET status = '2', jam_keluar = '$jam_keluar' WHERE kode = '$kode_keluar'");
      echo "<script>alert('Terima Kasih...');document.location.href='home.php?nama=$username'</script>";
    }
  }else{
